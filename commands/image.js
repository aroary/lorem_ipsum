...
async function execute() {
    ...
    if (!width || !height) return vscode.window.showErrorMessage(`Invalid width or height (${width}x${height})`);

    const text = [];
    while (text.length < lines) text.push(generate(Math.floor(Math.random() * 6) + 16).slice(8).join` `);

    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" overflow="hidden" role="img" aria-label="lorem_ipsum"><title>lorem ipsum</title>
